...
2. Serve the website: `bundle exec jekyll serve --baseurl ''`. If you want, you can speedup the process by loading only the last blog post instead of all of them. Simply add `--limit_posts 1` to the command above. The resulting command will be `bundle exec jekyll serve --limit_posts 1 --baseurl ''`.
