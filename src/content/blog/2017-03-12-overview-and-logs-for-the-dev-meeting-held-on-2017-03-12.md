...
**\<fluffypony>** well let's be brutally honest here: we didn't write this code, and parts of it are well-written whilst others are simply _not_  
...
**\<hyc>** hmmm. binaryFate - in that regard, C++ _is_ a superset of C. so if you can't recognize underhanded code in C, you wouldn't have seen it in C++ either.  
...
**\<fluffypony>** yes  
**\<fluffypony>** so the in-memory interface went from being _the thing you use_ to having all its functions updated to exist behind blockchainDB  
