...
**\<fluffypony>** moneromooo: it _is_ needed for the signature, not everyone is signing their commits  
...
**\<fluffypony>** olark: that's not the problem, my merge commits are GPG signed, so _I'm_ the failure mode, not Github  
