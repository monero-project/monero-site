...
**\<sarang>** Our use of hash functions is about as uniform as you can expect (slight bias with H_s)  
...
**\<suraeNoether>** on the other hand, the problem can be avoided by computing the mask as H('mask', z, i, P_output) where P_output is the output key being masked.  
...
**\<sarang>** Right now encrypted_amount = amount + H(z)  
**\<oneiric\_>** :)  
...
**\<knaccc>** moo has reimplemented that as encrypted_amount = keccak("amount" || shared secret)  
**\<knaccc>** mooo\*  
**\<sarang>** Sure, that's even better  
**\<sarang>** Er, wait  
**\<sarang>** that's the final encrypted amount sent to the recipient?  
**\<knaccc>** oops i left out tons. i meant encrypted_amount = 8 byte amount XOR 8 bytes of keccak("amount" || shared secret)  
**\<sarang>** aha  
**\<sarang>** yes  
...
**\<knaccc>** yes the entire thing is encrypted_amount = 8 byte amount XOR 8 bytes of keccak("amount" || Hs(8aR||i))  
...
**\<suraeNoether>** sarang: since pay_id isn't necessarily a security issue, but they are so freaking annoying, where do you stand on timeline for deprecation?  
