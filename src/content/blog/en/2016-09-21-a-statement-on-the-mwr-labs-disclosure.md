...
More importantly, **_since monero-wallet-cli does not enable RPC mode by default, there is nothing that is vulnerable in it unless the user actively enables a setting that allows for someone to remotely control the wallet_**.

We trust that MWR Labs will note this in their advisory, and adjust it accordingly.

_- The Monero Core Team_
