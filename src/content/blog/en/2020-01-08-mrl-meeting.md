...
**\<n3ptune\>** we used this formula where NRT is Node Received Timestamp: prop_time_lower_bound(h) = MAX[NRT(h,1), NRT(h,2), NRT(h,3)] - MIN[NRT(h,1), NRT(h,2), NRT(h,3)]  
...
**\<n3ptune\>** Is there a known purpose for the null padding tag in tx_extra?  
...
**\<Isthmus\>** If we have only 2 nodes, then our propagation envelope will have a lot of variability, be topology dependent (assume archival nodes don't connect to each other), and just t_second - t_first  
...
**\<n3ptune\>** --block-notify waits until the block is committed \>\> oh then i guess this method of doing it in blockchain::add_new_block() at least occurs before block-notify would. but yes it isn't \*immediately\* on receive  
...
**\<Isthmus\>** Oh yea  
**\<Isthmus\>** just a quick note that tx_padding is used in the wild, but unclear why  
...
**\<Isthmus\>** Turns out some (now fingerprinted) miners are using lots of null padding, check out the tx_extra for this coinbase  
**\<sarang\>** gadzooks  
**\<Snipa\>** Not super surprising.  
**\<Isthmus\>** Looks like bloat to me  
**\<Isthmus\>** Bunch of blocks waste space with nulls in tx_extra  
