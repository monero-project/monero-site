...
**\<oneiric\_>** currently, i have separated the independently testable stuff into unit_tests  
**\<oneiric\_>** stuff that requires networking goes in net_tests  
**\<Corklander>** Ah, right. Well that makes sense to my brain now. :)  
**\<oneiric\_>** so far, haven't seen the need for a separate integration_test, but will if the need arises  
